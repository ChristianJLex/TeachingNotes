...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[axis lines = middle, xmin = -1, ymin = -1, xmax = 4, ymax = 4]
			\addplot[color = blue!60, thick] {0.5*x+0.5 };
			\draw[-{Stealth[scale = 1.4]}, thick, purple]
			 (axis cs: 2,1.5) -- (axis cs:1,3.5);
			 \node at (axis cs: 1+0.4,3.5) {$\vv{n}$};
			\filldraw[red!60] (axis cs:0.5,0.75) circle (2pt);
			\node at (axis cs:0.5,0.75-0.3) {$P$};
			\node at (axis cs: 1.25-0.4,3) {$Q$};
			\draw[-{Stealth[scale = 1.4]}, thick, blue!60]
			(axis cs: 2,1.5) -- (axis cs: 1.25,3);
			\filldraw[red!60] (axis cs: 1.25,3) circle (2pt);
			\draw[-{Stealth[scale= 1.4]}, thick, color = green]
			(axis cs: 0.5,0.75) -- (axis cs: 1.25,3); 
			\draw[-{Stealth[scale= 1.4]}, thick, color = green]
			(axis cs: 0.5+1.5,0.75+0.75) -- (axis cs: 1.25+1.5,3+0.75); 
			\draw[thick, dashed, color = blue!60]
