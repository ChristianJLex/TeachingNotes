...
\begin{exa}
	...
	\begin{align*}
		A(x) = 2x\cdot x = 2x^2,
	\end{align*}
	som er en potensfunktion, hvor $b = 2$ og $a = 2$. 
\end{exa}
...
\begin{align*}
	...
	&c) \ \sqrt{3}\cdot x^{3.2} &&d) \ 1\cdot x^{-0.5}  \\
	&e) \ x^1     &   &f) \ 0.1\cdot x^{\sqrt{2}}
\end{align*}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		grid = both, minor tick num = 4]
			...
			\addplot[color = olive, thick, domain = 0.2:5, samples = 100] {24*x^(-1)};
